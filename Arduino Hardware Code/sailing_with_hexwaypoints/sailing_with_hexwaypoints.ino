...
Description: Incorporated sailing with 6 waypoints in a hexagonal shape at the Sac Aquatic Center. 
The rudder controller is responsible for adjusting the boat orientation
...
float distMargin = 4.0; // margin of error for waypoints (defined as a radius in [meters]) 
...
/*  // define waypoints (sac aquatic center)
  ...
  targetLocation.latitude = 38.636537; // point 7 (back to 1
  targetLocation.longitude = -121.216988;
  hexWaypoints.push_back(targetLocation);
  */
 ...
 /* targetLocation.latitude =  38.537669; // point 1: lake spafford (closest to shore)
  ...
  targetLocation.latitude = 38.537490; // point 2 (formerly 3)
  targetLocation.longitude = -121.747851;
  triWaypoints.push_back(targetLocation);
  
  targetLocation.latitude = 38.537712; // point 3 (formerly 2)
  targetLocation.longitude = -121.747790;
  triWaypoints.push_back(targetLocation); */
  
  targetLocation.latitude =  38.53876876; // point 1: park behind brisa
  targetLocation.longitude = -121.72125244;
  triWaypoints.push_back(targetLocation);
  
  targetLocation.latitude = 38.53939819; // point 2 
  targetLocation.longitude = -121.72130584;
  triWaypoints.push_back(targetLocation);
  
  targetLocation.latitude = 38.53922653; // point 3
  targetLocation.longitude = -121.72087097;
  triWaypoints.push_back(targetLocation);
  
  targetLocation = triWaypoints[0]; // to ensure that targetLocation is set at 0
...
void loop() {
  ...
  if ((distanceWP <= distMargin) && (distanceWP >= 0)){
    reached = true;
    point = iterWP + 1;
    iterWP += 1;
  ...
  Serial.print(distanceWP); Serial.print(",");
  //Serial.print("targ lat: "); Serial.print(targetLocation.latitude, 6); Serial.print(","); 
  //Serial.print("targ long: "); Serial.print(targetLocation.longitude, 6); Serial.print(","); 
  Serial.print("desired path angle: "); Serial.print(desiredPath); Serial.print(",");
  Serial.print("rudder angle:"); Serial.print(rudderAngle); Serial.print(",");
  Serial.print("error:"); Serial.print(errorActual); Serial.print(",");
  Serial.print("mag:"); Serial.print(currHeading); Serial.print(",");
  Serial.print("test:\t");Serial.print(calculate_orientation(targetLocation, currentLocation));
  if (reached == true) {
      Serial.print("waypoint "); Serial.print(point); Serial.print(" reached!");
      reached = false;
  } else {
