...
float Kp = 2.5;           // proportional gain - starting value suggested by Davi
float Ki = 0.01;       // integral gain - starting value suggsted by Davi
...
float rudder_controller(float desiredPath, float heading) {
  ...
  // apply PI theory 
  controlAct = P() + I();
  angle = rudOffset + abs(rudderNeg- rudOffset)*(controlAct/180);
