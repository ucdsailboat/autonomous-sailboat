...
void loop() {
  ...
  // runs if the sailboat is heading towards the dead zone 
  if (CalDirection >= 174 || CalDirection <= -172) {
    ...
    distanceTK = calculate_distance(prevLocation, currentLocation);
    if (distanceTK < 15) {
      rudderAngle = rudder_controller(againstWindAngle,windAngle);
